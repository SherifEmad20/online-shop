...
function GetAdmins() {
  ...
  useEffect(()=>{
    axios.get(`http://localhost:18072/Company-1.0-SNAPSHOT/api/order/getPendingOrders/${localStorage.getItem('loggedIn')}`).then((response)=>{
  ...
  function Navigate(){
    if(flag){
      ...
      return <p className='retMes'>Order Confirmed</p>
        ...
        listOfAdmins.map((value, key)=>{
          return <tr>
             ...
             <td><button className='bt3' onClick={()=>{
              ...
              window.location='/reqShipping';
                
               
               

             }}>Request Shipping</button></td>
